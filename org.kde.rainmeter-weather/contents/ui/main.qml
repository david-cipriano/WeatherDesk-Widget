...
PlasmoidItem {
    ...
    property int currentTempMax: 0
    property int currentTempMin: 0
    ...
    onRefreshMinutesChanged: refreshTimer.interval = Math.max(1, root.refreshMinutes) * 60 * 1000
    ...
    function iconSource(iconCode) {
        ...
        return Qt.resolvedUrl("../images/weather-icons/" + iconCode + ".png")
    }
    ...
    function fetchWeather() {
        ...
        xhr.onreadystatechange = function () {
            ...
            // IMPORTANT (Option B): clear the ListModel every fetch
            forecastModel.clear()
            ...
            const maxArr = Array.isArray(daily.calendarDayTemperatureMax) ? daily.calendarDayTemperatureMax : []
            const minArr = Array.isArray(daily.calendarDayTemperatureMin) ? daily.calendarDayTemperatureMin : []
            ...
            if (maxArr.length > 0) root.currentTempMax = maxArr[0]
            if (minArr.length > 0) root.currentTempMin = minArr[0]
            ...
            // Fill 6-day forecast model for your 3x2 grid
            const count = Math.min(6, days.length, maxArr.length, minArr.length)
            ...
            if (headLines && Array.isArray(headLines.alerts)) {
                for (let i = 0; i < headLines.alerts.length; i++) {
                    root.alertString = headLines.alerts[i].headlineText + "\n"
                }
            }
        }
        xhr.send()
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    PlasmaComponents.Label {
                        text: "<font color='red'><b>↑</b></font>" + root.currentTempMax + "°"
                        font.family: root.widgetFont.family
                        font.pointSize: 12
                    }
                    PlasmaComponents.Label {
                        text: "<font color='cyan'><b>↓</b></font>" + root.currentTempMin + "°"
                        font.family: root.widgetFont.family
                        font.pointSize: 12
                    }
                }
            ...
            Item {
                Layout.preferredWidth: 168  //Math.max(120, root.width * 0.38)
